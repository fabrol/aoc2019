...
def get_positions(line) -> List[Tuple[int, int, int]]:
    cur_pos = [0, 0, 0]
    positions = set([])
    ...
    for step in line:
        ...
        # print(cur_pos, direction, num_steps)
        if direction == "L":
            new_pos =[(cur_pos[0] - i, cur_pos[1], i + cur_pos[2]) for i in range(num_steps + 1)]
            ...
            new_pos =[(cur_pos[0], cur_pos[1] + i, i + cur_pos[2]) for i in range(num_steps + 1)]
            positions.update(new_pos)
            cur_pos[1] += num_steps
        elif direction == "D":
            new_pos =[(cur_pos[0], cur_pos[1] - i, i + cur_pos[2]) for i in range(num_steps + 1)]
            ...
            print ("DA FUQ U SAY TO ME?")
        # Update the distance
        cur_pos[2] += num_steps
            
    positions.remove((0,0,0))
    return positions

def get_distance(position, d1, d2):
    if position not in d1 or position not in d2:
        print('fuck')
    return abs(d1[position]) + abs(d2[position])

def process_line(line):    
    line_points = line.split(",")
    all_positions = get_positions(line_points)  # list of [(x, y, d)]
    
    # Separate into a dict {(x, y): d} and a set of (x,y) positions
    distances = {}
    positions = set()
    for x, y, d in all_positions:
        if (x, y) not in distances or distances[(x, y)] > d:
            distances[(x, y)] = d
        positions.add((x, y))
...
def run(line1, line2):
    p1, d1 = process_line(line1)
    p2, d2 = process_line(line2)
    
    intersections = p1.intersection(p2)
    distances = [get_distance(x, d1, d2) for x in intersections]
...
def run_testcase(line1, line2, expected):
    actual = run(line1, line2)
    if actual != expected:
        print("ERROR: %s %s. Expected %s, Found %s" % (line1, line2, expected, actual))
...
print(run(input_lines[0], input_lines[1]))
